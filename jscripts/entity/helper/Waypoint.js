namespace.module('vd.entity.helper', function(exports, require) {
    ...
    /**
    * Describes a waypoint (of a flight, SID, STAR).
    * @constructor
    ...
    */
    ...
    exports.Waypoint.prototype._draw = function(forceRedraw) {
        ...
        var marker = this._getMarker(latLng, title);
        var label = this._getLabel(latLng, title);
    ...
    // @param {String} title
    exports.Waypoint.prototype._getMarker = function(latLng, title) {
        var m = null;
        var imageUrl = null;
        switch (this.type) {
        case vd.entity.helper.WaypointSettings.TypeFlight:
            break;
        case vd.entity.helper.WaypointSettings.TypeFlightplan:
            break;
        default:
            break;
        }
        if (!String.isNullOrEmpty(imageUrl)) {
            var wpIcon = new google.maps.MarkerImage(imageUrl, new google.maps.Size(16, 16),
    ...
    // @param {google.maps.LatLng} latLng
    // @param {String} title
    exports.Waypoint.prototype._getLabel = function(latLng, title) {
    ...
    // @private
    // @return {String}
    exports.Waypoint.prototype._getTitle = function() {
        var title = "";
        switch (this.type) {
        case vd.entity.helper.WaypointSettings.TypeFlight:
            if (this.waypointSettings.displayFlightCallsign) title += this.name;
            if (this.waypointSettings.displayFlightAltitudeSpeed) title = title.appendIfThisIsNotEmpty("<br>") + this.groundspeedAndUnit() + " " + this.altitudeAndUnit();
            break;
        default:
            title = this.name;
            break;
